// gcc -g  -o liveping liveping.c -lpthread
...
int send_ping_wrapper(struct record *e, int family, int bufsize) {
	...
	if (family == AF_INET) {
		numbytes = send_ping(raw_fd, snd_buf, e->seq, bufsize, e);
	}
	else {
		numbytes = send_ping6(raw_fd, snd_buf, e->seq, bufsize, e);
	}
...
int main(int argc, char *argv[])
{
	...
	FILE *fp1 = fopen("/tmp/ping_res_OK.txt", "w");
	FILE *fp2 = fopen("/tmp/ping_res_Fail.txt", "w");
	int len = 0;
	for(i = 0; i < nLine; i++) {
		struct record *e = iplist[i];
		len = snprintf(tmpbuf, sizeof(tmpbuf), "%s %d %d\n",
				e->name, e->seq, e->latency);
		if(e->latency == -1) {
			fwrite(tmpbuf, len, 1, fp2);
		} else {
			fwrite(tmpbuf, len, 1, fp1);
		}
	}
	fclose(fp1);
	fclose(fp2);
