        ...
        uint16_t sndlen, long *ts)
{
    ...
    *ts = t_pkt->tv_sec * 1000 + t_pkt->tv_usec / 1000;
...
//checksum)
static ssize_t handle_ping_reply(struct record *e, uint16_t bufsize,
        int16_t family)
{
	int32_t icmp_fd = e->fd;
    ssize_t numbytes = -1;
...
int send_ping_wrapper(struct record *e, int family, int bufsize) {
	...
	*pseq = 0;
	if (family == AF_INET) {
		numbytes = send_ping(raw_fd, snd_buf, *pseq, bufsize, &(e->last_send));
	}
	else {
		numbytes = send_ping6(raw_fd, snd_buf, *pseq, bufsize, &(e->last_send));
	}
...
void * do_work(void *data) {
	...
	int i = 0, epoll_fd = targ->epollfd, raw_fd = -1;
    ...
    for(i = 0; i < targ->n_elems; i++) {
    	...
    	raw_fd = create_icmp_socket(e->name, g_local, g_family, g_protocol, g_iface);
		if (raw_fd == -1) {
			exit(EXIT_FAILURE);
		}
		e->fd = raw_fd;
	    ...
	    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, raw_fd, &ev) == -1) {
	        perror("epoll_ctl");
			exit(EXIT_FAILURE);
	    }
		e->epfd = epoll_fd;
    ...
    // 50Hz
    int32_t sleep_time = 20;
    int32_t nfds = 0;

	struct epoll_event ev[NR_MAX_POLL];

	while (1) {
		//Wait for reply or timeout, set to 10ms
		nfds = epoll_wait(epoll_fd, ev, NR_MAX_POLL, sleep_time);
		...
		if (nfds == -1) {
			...
			//nLoops++;
			//fprintf(stdout, " *** Idle nLoops=%d nGot=%d\n", nLoops, nGot);
		} else {
			for(i = 0; i < nfds; i++) {
				struct record *e = ev[i].data.ptr;
				if (handle_ping_reply(e, g_bufsize, g_family) > 0) {
					;
				}
			}
		}
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
    	...
    	if(ts > last_report_ts + 10000) {
    		fprintf(stdout, " *** %ld: %d out of %d not reply, total %ld send, loop %d\n",
    				ts, nlost, nLine, n_ping_send, nloop);
    		nloop = 0;
    		n_ping_send = 0;
    		last_report_ts = ts;
    	}
