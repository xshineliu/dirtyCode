...
#define EXT_DATA_BLOCK_SIZE (2 * 1024 * 1024) // 2 MiB MAX
...
struct {
	...
	char log_idf[32];
	char *prog;
} cfg = { .nWorkers = 1, .nr_dir = 1, .max_file_size = 128UL * (1UL << 20),
...
int session_set_on_disk_log_filename(struct session *s) {
    ...
    // code is needed for short live connection (for long connection: statbuf.st_size in most case is zero)
	...
	if(s->rough_file_offset > cfg.max_file_size) {
		...
		if(fd < 0) {
		    fprintf(stderr, "[ERROR] File %s can not be opened for writing: %s\n",
				fileName, strerror(errno));
			return -1;
		}
	...
	s->on_disk_fd = fd;
	// s->rough_file_offset set already above in this function
	return fd;
}
...
int enlarge_session_buf(struct session *s, int gap) {
	...
	if (newlen > MAX_DATA_BLOCK_SIZE) {
		...
		// net_read_buf_len modified is fine, just for net receive/read, mem buf not changed
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "vp:a:n:m:l:d:qt:k:s:h")) != -1) {
		...
		case 'k':
			...
			break;
		case 's':
			// file size limit as MiB
			cfg.max_file_size = atoi(optarg) * (1UL << 20);
			break;
		case 'h':
		default:
			usage();
	...
	fprintf(stderr, "[INFO] %s start with %d worker threads, %d directories, max file size %d MiB\n",
			cfg.prog, cfg.nWorkers, cfg.nr_dir, cfg.max_file_size / (1UL << 20));
